...
from .metrics import (
    ...
    RefinementMetricReport,
    compare_refinement_metrics,
)
...
_LOWER_IS_BETTER = {
    ...
    "out_of_page_count",
    "off_grid_geometry_count",
    "non_junction_wire_crossing_count",
    "total_wire_manhattan_length_mm",
    "bend_count",
    "excessive_bend_wire_count",
    "long_wire_count",
    ...
    "alignment_residual_mean_mm",
    "distinct_x_columns",
}

# Session-level best-known replacement deliberately uses Pareto dominance instead of a
# weighted aggregate. Only metrics with an already-explicit direction in the deterministic
# acceptance contract participate, so L2 does not invent new scoring semantics.
BEST_KNOWN_LOWER_IS_BETTER = frozenset(_LOWER_IS_BETTER)
...
def evaluate_candidate_quality(
    ...
    *,
    ...
    regressions = [
        name for name in PROTECTED_METRICS if getattr(after, name) > getattr(before, name)
    ]
    if regressions:
        return CandidateQualityDecision(
            False,
            "REFINEMENT_PROTECTED_METRIC_REGRESSION",
            f"protected metric regression: {', '.join(regressions)}",
            (),
            comparison,
        )
    ...
    targeted = {
        ...
        for metric in _CATEGORY_METRICS.get(category, ())
    }
    improved: list[str] = []
    ...
    if not improved:
        return CandidateQualityDecision(
            ...
            "no addressed deterministic metric improved",
            (),
            comparison,
        )
    return CandidateQualityDecision(
