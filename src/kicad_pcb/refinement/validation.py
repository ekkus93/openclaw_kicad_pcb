...
def validate_candidate_structure(candidate: Path, *, adapter: KicadCliAdapter, work_dir: Path | None = None) -> CandidateStructuralValidationReport:
    doc = SchematicDoc.load(candidate)
    lint_errors = tuple(sorted(issue.code for issue in lint_schematic(doc.root) if issue.severity is LintSeverity.ERROR))
    if lint_errors:
    ...
    try:
        ...
        if not isinstance(report, dict) or not isinstance(report.get("violations"), list):
            raise UserError("KiCad ERC did not produce a usable violation report.", code="REFINEMENT_ERC_INVALID_REPORT")
        count = len(report["violations"])
        return CandidateStructuralValidationReport("1.0", "passed" if count == 0 else "failed", (), count)
    finally:
        try:
            Path(raw).unlink()
            ...
            LOGGER.warning("failed to remove temporary refinement ERC report", extra={"error_type": type(exc).__name__})
