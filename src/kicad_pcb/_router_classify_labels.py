...
def _requires_named_signal_identity(
    classification: Literal[
        "power",
        "local_decoupling",
        "shunt_ground",
    ...
    """Return True when KiCad needs an attached label to retain the authored net name.

    Wires alone preserve connectivity but not an authored signal-net name: KiCad
    assigns an autogenerated name to an otherwise unlabeled signal. Hidden bind
    markers describe generator intent only and do not participate in KiCad's
    electrical graph. Power/ground classes retain their dedicated symbol/label
    routing semantics instead of using this signal-net identity label.
    """
    return classification not in {"power", "local_decoupling", "shunt_ground"}
