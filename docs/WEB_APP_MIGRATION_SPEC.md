...
The first v1 implementation may run generation synchronously inside `POST /api/jobs/from-netlist`. If synchronous generation is used, still create a job and persist `job.json` before doing work.

The endpoint must return the final job detail if synchronous, or the queued job summary if a background worker is added.
