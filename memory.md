...
## 2026-08-16T18:51:35Z - GPT-5.6 Sol - Added the Phase K real apply_once integration gate

- Added `tests/integration/test_refinement_phase_k_apply_once_real_kicad.py` in commit `cb611169921fcf32f08634d31f4c8bee07ff180f`. The success case drives the production `apply_once_schematic_refinement` path with only the LLM transport and PNG rasterization deterministic/fake; schematic generation, registered wire mutation, KiCad SVG export, electrical verification/netlist export, ERC/structural validation, metric acceptance, evidence, and atomic promotion remain production-real.
- The real fixture starts from the proven two-resistor LEFT/MID/RIGHT schematic and adds a legal orthogonal detour on `MID`; locally that raises total wire Manhattan length from `63.5` to `73.66` mm and bend count from `2` to `3`, giving the model-directed `shorten_wire_path` repair an objective deterministic improvement target.
- A second real-fixture case intentionally returns stale wire geometry from the fake planner and asserts the accepted schematic remains byte-identical. Existing K4 failure-injection coverage remains the broader safety-failure matrix; this new case adds real-fixture transactional preservation evidence.
- Local evidence before publish: the new file compiled, collected `2` integration tests, the focused refinement/service regression slice passed `52/52`, and the detour metric probe passed. Local `kicad-cli` is unavailable, so the Phase K hard-exit checkbox must remain open until the normal `requires_kicad` integration run executes these tests successfully.
