...
class _Adapter:
    ...
    def export_svg_sch(self, _sch: Path, output: Path):
        output.write_text('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 297 210"></svg>', encoding="utf-8")
        return _Result()


class _Rasterizer:
    def rasterize(self, _svg: Path, png: Path) -> None:
        png.write_bytes(b"\x89PNG\r\n\x1a\n" + b"\x00\x00\x00\x0dIHDR" + (297).to_bytes(4,"big") + (210).to_bytes(4,"big") + b"\x08\x02\x00\x00\x00")
...
def test_render_binds_hashes_and_transform(tmp_path: Path) -> None:
    ...
    artifact = render_schematic_for_refinement(fixture, tmp_path / "out", adapter=_Adapter(), rasterizer=_Rasterizer())  # type: ignore[arg-type]
