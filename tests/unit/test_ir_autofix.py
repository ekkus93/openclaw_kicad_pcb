...
def test_autofix_repairs_nodes_key_and_infers_common_missing_symbols() -> None:
    outcome = autofix_circuit_ir(
        {
            "version": "1",
            "components": [
                {"ref": "U1", "value": "NE555", "footprint": "Package_DIP:DIP-8_W7.62mm"},
                {"ref": "R1", "value": "68k"},
                {"ref": "C1", "value": "10uF", "footprint": "Capacitor_SMD:CP_Elec_5x5.8"},
                {"ref": "D1", "value": "Red LED", "footprint": "LED_SMD:LED_0805_2012Metric"},
            ],
            "nets": [
                {
                    "name": "VCC",
                    "nodes": [{"ref": "U1", "pin": 8}, {"ref": "R1", "pin": 1}],
                },
                {
                    "name": "GND",
                    "nodes": [
    ...
    assert outcome.ir_dict["components"] == [
        ...
        {"ref": "R1", "value": "68k", "symbol": "Device:R"},
        {
            "ref": "C1",
            "value": "10uF",
            "footprint": "Capacitor_SMD:CP_Elec_5x5.8",
    ...
    assert 'U1: inferred missing symbol as "Timer:NE555"' in outcome.fixes_applied
    assert 'net "VCC": renamed membership key "nodes" -> "pins"' in outcome.fixes_applied


def test_autofix_repairs_compact_node_tokens_and_strips_invalid_options() -> None:
    outcome = autofix_circuit_ir(
        {
            "version": "1",
            "components": [
            ...
            "nets": [
                {"name": "VCC", "nodes": ["U1.8", "U1.4", "R1.1"]},
                {"name": "OUT", "nodes": ["U1.3", "D1.1"]},
            ],
            "options": {"notes": ["generated by llm"]},
        }
    )

    assert outcome.ir_dict == {
        "version": "1",
        "components": [
            {
                "ref": "U1",
