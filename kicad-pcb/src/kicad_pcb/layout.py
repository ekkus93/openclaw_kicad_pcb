...
# Grid constants (mm)
# ---------------------------------------------------------------------------
GRID_COL_MM: float = 40.64  # 1600 mil column width — space for component + stubs
GRID_ROW_MM: float = 25.40  # 1000 mil row pitch (one inch)
...
def compute_signal_flow_layout(ir: CircuitIR) -> dict[str, tuple[float, float]]:
    ...
    for c_num, members in sorted(by_col.items()):
        ...
        for row, ref in enumerate(members):
            x = ORIGIN_X + c_num * GRID_COL_MM
