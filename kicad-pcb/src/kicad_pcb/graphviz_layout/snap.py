...
def _opamp_stage_input_pair(
    ...
    *,
    ...
    ``OPAMP_CORE`` unit to exactly two nearby support refs, where exactly one of
    those refs also connects to ground. That corresponds to a typical
    non-inverting stage handoff with an incoming bridge element and a local
    shunt-to-ground support element at the positive input.
    """
...
def _snap_opamp_stage_non_inverting_input_node_shape(
    ...
    *,
    ...
    A gain stage reads more clearly when the final input handoff stays on the
    op-amp row and the local shunt support drops one row below the non-inverting
    input node, rather than leaving that pair staggered by generic input-stage
    lane spreading.
    """
